...
@objc final class MetricCollectionManager: NSObject, MXMetricManagerSubscriber, @unchecked Sendable {
    ...
    private func recordSchedulingLatency(identifier: String, latency: TimeInterval) {
        let event = BackgroundTaskEvent(
            ...
            errorMessage: nil,
            metadata: [
                "scheduling_latency": String(latency),
    ...
    private func createEnhancedTaskEvent(
        ...
        let stringMetadata = convertMetadataToStringDictionary(metadata)
    ...
    // Helper function to convert [String: Any] to [String: String]
    private func convertMetadataToStringDictionary(_ metadata: [String: Any]) -> [String: String] {
        ...
        for (key, value) in metadata {
            ...
            case let int64Value as Int64:
                stringMetadata[key] = String(int64Value)
            case let doubleValue as Double:
                stringMetadata[key] = String(format: "%.3f", doubleValue)
            case let floatValue as Float:
                stringMetadata[key] = String(format: "%.3f", floatValue)
            case let boolValue as Bool:
                stringMetadata[key] = String(boolValue)
            case let timeIntervalValue as TimeInterval:
                stringMetadata[key] = String(format: "%.3f", timeIntervalValue)
            default:
    ...
    private func processMetricPayload(_ payload: MXMetricPayload) {
        let metricData = extractMetricKitData(from: payload)
        let stringMetadata = convertMetadataToStringDictionary(metricData)
    ...
    private func processDiagnosticPayload(_ payload: MXDiagnosticPayload) {
        let diagnosticData = extractDiagnosticData(from: payload)
        let stringMetadata = convertMetadataToStringDictionary(diagnosticData)
